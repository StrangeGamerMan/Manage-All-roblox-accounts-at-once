...
const apiUrl = (p) => API_BASE.replace(/\/$/, '') + (p.startsWith('/') ? p : '/' + p);
...
function updateAccountCount() {
  accountCount.textContent = parseAccounts(accountsTextarea.value).length;
}
function setLoading(isLoading) {
  if (isLoading) { runBtn.classList.add('loading'); runBtn.setAttribute('disabled', 'true'); }
  else { runBtn.classList.remove('loading'); runBtn.removeAttribute('disabled'); }
}
...
async function run() {
  ...
  if (!accounts.length) { showToast('Add at least one account: Label|.ROBLOSECURITY cookie', 'err'); accountsTextarea.focus(); return; }

  output.textContent = '';
  append(`Using API: ${API_BASE}`, 'muted');
  append(`Running "${cmd}" for ${accounts.length} account(s)...`, 'muted');
  setLoading(true); setProgress(18);
...
chips.forEach(ch => ch.addEventListener('click', () => {
  commandInput.value = ch.getAttribute('data-example'); commandInput.focus();
}));
...
const saveBtnEl = document.getElementById('saveAccounts');
const clearBtnEl = document.getElementById('clearAccounts');
saveBtnEl.addEventListener('click', saveToStorage);
clearBtnEl.addEventListener('click', () => {
  accountsTextarea.value = ''; updateAccountCount(); saveToStorage();
});
...
async function pingBackend() {
  try {
    ...
    append(res.ok ? `Backend online at ${API_BASE}` : `Backend responded ${res.status} at ${API_BASE}`, res.ok ? 'ok' : 'err');
  } catch {
